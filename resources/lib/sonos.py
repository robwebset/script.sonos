...
class Sonos(SoCo):
    ...
    def setGroupVolume(self, volumeValue, forceIdenticalVolume=False):
        ...
        if activeGroup not in [None, ""]:
            for member in activeGroup.members:
                # Check if we are forcing to exactly the same volume of if we are
                # trying to keep some form of offset
                if forceIdenticalVolume:
                    member.volume = volumeValue
                else:
